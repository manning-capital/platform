...
import { Trade, QuantModel, DashboardStats, TradeChartData, TimeSeriesPoint, SignalLevel, TradeMetrics, MetricData, StrategyCondition } from '../models/trade.model';
...
export class TradingService {
  ...
  private generateChartDataForStrategy(trade: Trade, strategy: string, metricName: string): TradeChartData {
    ...
    const currentValue = metricName === 'Price' ? trade.currentPrice : timeSeries[timeSeries.length - 1].value;
    timeSeries.push({ 
    ...
    switch (strategy) {
      case 'Mean Reversion':
        ...
        conditions.push({
          ...
          currentValue: currentPrice.toFixed(2),
          targetValue: (basePrice * (trade.side === 'BUY' ? 0.97 : 1.03)).toFixed(2),
          operator: trade.side === 'BUY' ? 'above' : 'below',
          isMet: trade.side === 'BUY' ? currentPrice > basePrice * 0.97 : currentPrice < basePrice * 1.03,
          type: 'stopLoss'
        });
        break;
      case 'Trend Following':
        conditions.push({
          ...
          currentValue: trade.side === 'BUY' ? 'Uptrend' : 'Downtrend',
        ...
        conditions.push({
          ...
          targetValue: (trade.side === 'BUY' ? '+8%' : '-8%'),
          operator: trade.side === 'BUY' ? 'above' : 'below',
          isMet: trade.side === 'BUY' ? currentPrice >= basePrice * 1.08 : currentPrice <= basePrice * 0.92,
          type: 'exit'
        });
        break;
      case 'Options Strategy':
        ...
        conditions.push({
          ...
          targetValue: '+3%',
          operator: 'above',
          isMet: trade.side === 'BUY' ? currentPrice >= basePrice * 1.03 : currentPrice <= basePrice * 0.97,
          type: 'exit'
        });
        break;
      case 'Market Making':
        ...
        conditions.push({
          ...
          currentValue: trade.quantity.toString(),
          targetValue: '< 150',
          operator: 'below',
          isMet: trade.quantity < 150,
          type: 'exit'
        });
        conditions.push({
          id: 'price-deviation',
